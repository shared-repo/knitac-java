...
public class Ex05Array {
	...
	public static void main(String[] args) {
		//1. 배열 만들기
		// int a;
		int[] ar = null; // 참조 변수 만들기, null : 참조하는 인스턴스가 없음을 의미하는 값
		ar = new int[10]; // 요소가 10개인 배열 인스턴스 만들기 + 인스턴스의 주소를 ar 참조변수에 저장
		// int[] ar = new int[10]; //int 데이터 10개를 관리하는 배열
		
		//2. 배열 요소 사용
		System.out.printf("%d-%d\n", ar[0], ar[1]); // 정수형 배열 요소의 값은 0으로 초기화
		
//		for (int i = 0; i < 10; i++) { // 1 : 0 ~ 9
		for (int i = 0; i < ar.length; i++) { // 배열.length : 배열에 포함된 요소의 갯수
			ar[i] = (int)(Math.random() * 900) + 100; // 100 ~ 1000(미포함)
		}
